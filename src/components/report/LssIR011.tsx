...
import { getCommon, LssIR011ListWhere, LssIR011List, downloadFilePost } from "../../data";
...
export default function LssIR011() {
  ...
  const fnPost = useCallback(async (COMPANY_CODE: string, YEAR: string, QUARTER: string) => {
    ...
    //console.log(para);
    LssIR011ListWhere(para).then((data: any) => {
      let tmpSchema = [];
      let tmpSchemaSum = [];
      //console.log(data);
      var TempCode = "";
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].TYPE == "1") {
          ...
          TmpSumT1COL1 = round(TmpSumT1COL1 + round(parseFloat(data[i].COL1 == null ? 0 : data[i].COL1)));
          TmpSumT1COL2 = round(TmpSumT1COL2 + round(parseFloat(data[i].COL2 == null ? 0 : data[i].COL2)));
          TmpSumT1COL3 = round(TmpSumT1COL3 + round(parseFloat(data[i].COL3 == null ? 0 : data[i].COL3)));
          TmpSumT1COL4 = round(TmpSumT1COL4 + round(parseFloat(data[i].COL4 == null ? 0 : data[i].COL4)));
          TmpSumT1COL5 = round(TmpSumT1COL5 + round(parseFloat(data[i].COL5 == null ? 0 : data[i].COL5)));
          TmpSumT1COL6 = round(TmpSumT1COL6 + round(parseFloat(data[i].COL6 == null ? 0 : data[i].COL6)));
          TmpSumT1COL7 = round(TmpSumT1COL7 + round(parseFloat(data[i].COL7 == null ? 0 : data[i].COL7)));
          TmpSumT1COL8 = round(TmpSumT1COL8 + round(parseFloat(data[i].COL8 == null ? 0 : data[i].COL8)));
          TmpSumT1COL9 = round(TmpSumT1COL9 + round(parseFloat(data[i].COL9 == null ? 0 : data[i].COL9)));
          ...
          for (let j = 0; j < data.length; j++) {
            if (data[j].TYPE == "2" && data[i].COMPANY_CODE == data[j].COMPANY_CODE
              ...
              console.log('TYPE : ' + data[j].TYPE + 'COMPANY_CODE : ' + data[j].COMPANY_CODE
                + 'YEAR : ' + data[j].YEAR + 'QUARTER : ' + data[j].QUARTER)
              tmpSchema.push({
                id: Tempid,
              ...
              TmpSumT2COL1 = round(TmpSumT2COL1 + round(parseFloat(data[j].COL1)));
              TmpSumT2COL2 = round(TmpSumT2COL2 + round(parseFloat(data[j].COL2)));
              TmpSumT2COL3 = round(TmpSumT2COL3 + round(parseFloat(data[j].COL3)));
              TmpSumT2COL4 = round(TmpSumT2COL4 + round(parseFloat(data[j].COL4)));
              TmpSumT2COL5 = round(TmpSumT2COL5 + round(parseFloat(data[j].COL5)));
              TmpSumT2COL6 = round(TmpSumT2COL6 + round(parseFloat(data[j].COL6)));
              TmpSumT2COL7 = round(TmpSumT2COL7 + round(parseFloat(data[j].COL7)));
              TmpSumT2COL8 = round(TmpSumT2COL8 + round(parseFloat(data[j].COL8)));
              TmpSumT2COL9 = round(TmpSumT2COL9 + round(parseFloat(data[j].COL9)));
              Tempid = Tempid + 1;
              TmpCOL1 = 0; TmpCOL2 = 0; TmpCOL3 = 0; TmpCOL4 = 0; TmpCOL5 = 0; TmpCOL6 = 0; TmpCOL7 = 0; TmpCOL8 = 0; TmpCOL9 = 0;
            }
          }
  ...
  //   }
  //   downloadFilePost(
  //     `/LSSIR011/LSSIR011Excel`,
  //     "LSS-IR-011_CRR_(แสดงรายละเอียดการคำนวณ)_ประกันชีวิตฯ.xlsx",
  //     record
  //   );
  // };
      ...
      <div>
        ...
        <div style={{ width: "100%", marginTop: 100 }}>
          <h5>
            ...
            &nbsp;&nbsp; LSS-IR-011 รายงานแสดงการเปรียบเทียบข้อมูลเงินสมทบ
            ระหว่างยอดเงินสมทบตามการยื่นแบบกับยอดเงินสมทบที่คำนวณ<br />ได้จากเบี้ยประกันภัยรับโดยตรงจากระบบ
            CRR (แสดงรายละเอียดการคำนวณ) ประกันชีวิต ฯ
          </h5>
            ...
            <Row style={{ marginBottom: 10, marginTop: 20 }}>
              ...
              >
                <ExportExcelVarious
                  ...
                  IDPage={"LSS-IR-011"}
                  fileNames={"LSS-IR-011_CRR_(แสดงรายละเอียดการคำนวณ)_ประกันชีวิตฯ.xlsx"}
                ></ExportExcelVarious>
                {/* <Button
                  style={{ marginRight: 5 }}
                  variant="contained"
