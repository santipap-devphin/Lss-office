...
import { donwloadFile, getCommon, getLSSIRA02, getCompanyList, getSectionList, LssITA60ListAPI, getLSSIRA05OnLoad, downloadFilePost } from "../../data";
...
export default function LSSIRA02() {
  const [gridData, setGridData] = useState<LSS_V_LSSIRA02[]>([]);
  ...
  const handleDownloadExcel = () => {
    ...
    if (companyType !== "0" && companyType !== "") record["COMPANY_TYPE_CODE"] = companyType;
    downloadFilePost(
      `/LssIRA02/LSSIRA02Excel`,
      "LSS-IR-A02_รายงานการทำงานที่สำคัญ_(Transaction_log).xlsx",
      record
    );
  };
      ...
      <Row>
        <Col>
          ...
          <h5 style={{ marginTop: "7rem" }}>
            ...
            &nbsp;&nbsp;LSS-IR-A02 รายงานการทำงานที่สำคัญ (Transaction log)
          </h5>
          <Card>
            <Card.Header>
              LSS-IR-A02 รายงานการทำงานที่สำคัญ (Transaction log)
