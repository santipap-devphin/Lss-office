...
import { getCommon, CRR_NONLIFE_LIST ,BatchCRR ,CompanyList} from "../../data";
...
const LssITBatchCRR = () => {
  ...
  const handleOnLoad = () => {
    return new Promise((resolve, reject) => {
      let para = JSON.parse( `{"CODE":"NONLIFE"}`  );
  ...
  const fnLoad  = useCallback(async (para:any) => {
    CRR_NONLIFE_LIST(para).then((data) => {
       console.log(data);
      let tmpSchema = [];
      for (let i = 0; i < data.CRR_DETAIL.length; i++) {
        tmpSchema.push({
          ...
          COMPARE_CODE: data.CRR_DETAIL[i].COMPARE_CODE,
...
>([]);
  ...
  function Process() {
    var paracrr = JSON.parse(
      `{"COMPANY_CODE":"${company}","COMPANY_TYPE_CODE":"NONLIFE","YEAR":"${sYear}",  "QUARTER":"${sQuarter}" }`
    );
    BatchCRR(paracrr)
    .then((res: any) => {
          ...
          <Col>
            ...
            <Card style={{ marginTop: "7rem" }}>
              ...
              <Card.Body>
                ...
                >
                  {" "}
                  ประมวลผล CRR บริษัทประกันวินาศภัย
                </Button>
